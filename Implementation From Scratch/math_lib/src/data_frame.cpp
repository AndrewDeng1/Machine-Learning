...
DataFrame::DataFrame(size_t rows, size_t cols): m(vector<vector<double>>(rows, vector<double>(cols, 0))){}

DataFrame::DataFrame(const vector<vector<double>>arr): m(arr){}

DataFrame::DataFrame(const vector<double>arr): m(vector<vector<double>>(arr.size(), vector<double>(1, 0))){
...
DataFrame::DataFrame(const vector<vector<double>> arr){
...
DataFrame::DataFrame(const vector<vector<double>> arr, const vector<string> columns){
    setData(vector<vector<string>>(arr[0].size(), vector<string>(arr.size(), "")));
    for(size_t i=0; i<arr[0].size(); i++){
        for(size_t j=0; j<arr.size(); j++){
            getData()[i][j]=to_string(arr[j][i]);
