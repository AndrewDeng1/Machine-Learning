...
Matrix LogisticRegression::gradient(const Matrix& X, const Matrix& y) const {
    return 1.0/(double)X.getRows()*X.T()*(sigmoid(X*w)-y);
}

// "const" indicates method isn't allowed to modify member variables, and can only access other "const" methods
// Assumes X is R^(mxn)
void LogisticRegression::fit_gradient_descent(const Matrix& X, const vector<double>& y, size_t epochs, double learning_rate) {
    fit_gradient_descent(X, Matrix(y), epochs, learning_rate);
}
void LogisticRegression::fit_gradient_descent(const Matrix& X, const Matrix& y, size_t epochs, double learning_rate) {
...
void LogisticRegression::fit_stochastic_gradient_descent(const Matrix& X, const vector<double>& y, size_t epochs, double learning_rate, size_t mini_batch_size) {
    fit_stochastic_gradient_descent(X, Matrix(y), epochs, learning_rate, mini_batch_size);
}
void LogisticRegression::fit_stochastic_gradient_descent(const Matrix& X, const Matrix& y, size_t epochs, double learning_rate, size_t mini_batch_size) {
    generate_random_weights(X.getCols());
    for(size_t iter=1; iter<=epochs; iter++){
...
double LogisticRegression::inference(const vector<double>& x) const {
    return sigmoid(Matrix(x).T().dot(w));
}